...
impact_sensor::impact_sensor(bool pressure) : impulse(0.f) {
...
void impact_sensor::on_load(bool created, bool has_state) {
    this->update_appearance();
}

void impact_sensor::update_appearance() {
...
float impact_sensor::get_slider_snap(int s) {
    switch (s) {
...
float impact_sensor::get_slider_value(int s) {
    switch (s) {
...
void impact_sensor::on_slider_change(int s, float value) {
    if (s == 0) {
...
edevice *impact_sensor::solve_electronics() {
