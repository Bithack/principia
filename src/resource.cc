...
void resource::write_quickinfo(char *out) {
...
void resource::on_load(bool created, bool has_state) {
    this->set_resource_type(this->properties[0].v.i8);
    this->set_amount(this->properties[1].v.i);
}

float resource::get_slider_value(int s) {
    return (this->properties[1].v.i-1) / 14.f;
}

void resource::on_slider_change(int s, float value) {
    this->set_amount(1+(uint32_t)roundf(value * 14.f));
    G->show_numfeed(this->amount);
}

void resource::set_resource_type(uint8_t resource_type) {
...
void resource::set_amount(uint32_t amount) {
    this->amount = amount;
