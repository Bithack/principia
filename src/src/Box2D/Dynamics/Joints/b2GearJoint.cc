...
: b2Joint(def)
{
    ...
    /* XXX */
    b2Vec2 p = m_bodyB->GetPosition() - m_bodyA->GetPosition();
    p*=1.f/p.Length();
    float aa = atan2f(p.y, p.x);

    //float last_a = m_referenceAngleA;

    //float da = tmath_adist(last_a, aa);
    //aa = last_a + da;
	...
	if (m_typeA == e_revoluteJoint)
	{
		...
		m_localAnchorC = revolute->m_localAnchorA;
		m_localAnchorA = revolute->m_localAnchorB;
		m_referenceAngleA = revolute->m_referenceAngle + tmath_adist(revolute->m_referenceAngle, aa);
		m_localAxisC.SetZero();
	...
	if (m_typeB == e_revoluteJoint)
	{
		...
		m_localAnchorD = revolute->m_localAnchorA;
		m_localAnchorB = revolute->m_localAnchorB;
		m_referenceAngleB = revolute->m_referenceAngle + tmath_adist(revolute->m_referenceAngle, aa);
		m_localAxisD.SetZero();
...
void b2GearJoint::InitVelocityConstraints(const b2SolverData& data)
{
    ...
    m_referenceAngleA = aa;
    m_referenceAngleB = aa;
}
