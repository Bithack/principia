...
const struct tms_mesh *
__OLD__tms_meshfactory_get_cylinder(void)
{
    if (cylinder == 0) {
        float step = (M_PI*2)/(float)CQ;
        ...
        struct tms_gbuffer *vbuf = tms_gbuffer_alloc(sizeof(struct cvert) * CQ*4);
        //struct tms_gbuffer *ibuf = tms_gbuffer_alloc(sizeof(short) * (CQ*4 + 12));
        struct tms_gbuffer *ibuf = tms_gbuffer_alloc(sizeof(short) * 59);
        ...
        /* index backside */
        indices[0] = CQ*3;
        for (int x=0; x<CQ/2; x++) {
            indices[1+x*2] = CQ*4-(x+1);
            indices[1+x*2+1] = CQ*3+x+1;
        }

        /* index sides */
        indices[CQ] = indices[CQ-1];
        indices[CQ+1] = 0;
        indices[CQ+2] = 0;
        for (int x=0; x<CQ+1; x++) {
            indices[CQ+3+x*2] = (x%CQ)*2;
            indices[CQ+3+x*2+1] = (x%CQ)*2+1;
        }

        int offs = CQ+3+((CQ+1)*2);
        indices[offs] = indices[offs-1];
        indices[offs+1] = CQ*2;
        indices[offs+2] = CQ*2;
        offs+=3;
        for (int x=0; x<CQ/2; x++) {
            indices[offs+x*2+1] = CQ*3-(x+1);
            indices[offs+x*2] = CQ*2+x+1;
        }
        indices[offs+CQ+2] = CQ*3-1;
        ...
        cylinder = tms_mesh_alloc(va, ibuf);
        tms_mesh_set_primitive_type(cylinder, TMS_TRIANGLE_STRIP);
    }
