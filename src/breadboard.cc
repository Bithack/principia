...
void breadboard::set_layer(int z) {
    switch (z) {
...
void breadboard::on_slider_change(int s, float value) {
    this->properties[s].v.f=value*BR_MAX_SIZE;
    this->disconnect_all();
    this->update_size();
}

void breadboard::on_load(bool created, bool has_state) {
    this->update_size();
}

void breadboard::update_size() {
...
connection *breadboard::load_connection(connection &conn) {
    this->c[conn.o_index] = conn;
    return &this->c[conn.o_index];
}

float32 breadboard::ReportFixture(b2Fixture *f, const b2Vec2 &pt, const b2Vec2 &nor, float32 fraction) {
    if (f->IsSensor()) {
        return -1.f;
    }
...
bool breadboard::ReportFixture(b2Fixture *f) {
