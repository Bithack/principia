...
b2Body::b2Body(const b2BodyDef* bd, b2World* world)
{
    ...
    if (b2World::force_sleep) {
        m_flags |= e_forceSleepFlag1;
    }

	if (bd->awake && !b2World::force_sleep)
	{
		m_flags |= e_awakeFlag;
	}
	if (bd->active)
	...
	m_linearDamping = std::max(bd->linearDamping, world->m_min_linear_damping);
	m_angularDamping = std::max(bd->angularDamping, world->m_min_angular_damping);
...
{
	m_linearDamping = std::max(linearDamping, this->GetWorld()->m_min_linear_damping);
}

void b2Body::SetAngularDamping(float32 angularDamping)
{
	m_angularDamping = std::max(angularDamping, this->GetWorld()->m_min_angular_damping);
}
...
void b2Body::SetAwake(bool flag)
{
	if (flag)
	{
        if ((m_flags & (e_forceSleepFlag1 | e_forceSleepFlag2)) != 0) {
            return;
        }
		...
		{
            if (m_type == b2_dynamicBody) {
                this->GetWorld()->SleepListener->OnWakeup(this);
            }
			m_flags |= e_awakeFlag;
            m_sleepTime = 0.0f;
		}
	...
	else
	{
        if (m_flags & e_awakeFlag) {
            if (m_type == b2_dynamicBody) {
                this->GetWorld()->SleepListener->OnSleep(this);
            }
        }
